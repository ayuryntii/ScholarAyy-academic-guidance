...
{
    $data['title'] = 'Menu Koordinator';
    $data['user'] = $this->db->get_where('user', ['user_id' => $this->session->userdata('user_id')])->row_array();
    $data['user_data'] = $this->db->get_where('user_data', ['data_id' => $data['user']['data_id']])->row_array();

    // Proses upload tanda tangan dosen
    $upload_file = $_FILES['tanda_tangan']['name'];

    if ($upload_file) {
        $config['upload_path'] = './assets/File/tandatangan_dosen/';
        $config['allowed_types'] = 'gif|jpg|png|jpeg';
        $config['max_size'] = 2048; // 2MB
        $config['file_name'] = 'tandatangan_dosen_' . time(); // Nama file unik
        ...
        if ($this->upload->do_upload('tanda_tangan')) {
            ...
            $tanda_tangan = $uploadData['file_name'];

            // Mendapatkan ID terbaru dari tabel user
            $query = 'SELECT * FROM user ORDER BY data_id DESC LIMIT 1';
            $data_id = $this->db->query($query)->row_array();
            $id = $data_id['data_id'] + 1;
            ...
            $data1 = [
                ...
                'tanda_tangan_digital' => $tanda_tangan, // Simpan nama file tanda tangan di kolom yang baru
                'dos_id' => $id
            ];
            ...
            // Insert data ke tabel admin dan user
            ...
            // Jika upload gagal
            $this->session->set_flashdata('message_cor_data_dosen', '<div class="alert alert-danger" role="alert">
            Tanda tangan dosen gagal diunggah: ' . $this->upload->display_errors() . '
            </div>');
            redirect('cordinator/informasi_dosen_pembimbing');
        }
    }
...
public function edit($id)
{
    ...
    // Mengambil password yang ada jika tidak diubah
    ...
    // Cek apakah ada file tanda tangan yang diunggah saat edit
    if ($_FILES['tanda_tangan']['name']) {
        ...
        if ($this->upload->do_upload('tanda_tangan')) {
            ...
            $this->session->set_flashdata('message_cor_data_dosen', '<div class="alert alert-danger" role="alert">
            Tanda tangan dosen gagal diunggah saat mengedit: ' . $this->upload->display_errors() . '
            </div>');
            redirect('cordinator/informasi_dosen_pembimbing');
            return;
        }
    } else {
        // Jika tidak ada file baru yang diunggah, tetap gunakan file lama
        $dosen = $this->db->get_where('admin', ['dos_id' => $id])->row_array();
        $tanda_tangan = $dosen['tanda_tangan_digital']; // Menggunakan kolom baru
    }
